# -*- coding: utf-8 -*-
"""
Fase 4 — Consultas SPARQL (>= 30) sobre a ontologia do Taim.

Carrega `taim.owl` com **rdflib** (SPARQL 1.1 completo: FILTER, COUNT, GROUP BY,
AVG, SUM, property paths) e executa 32 consultas distribuídas nas 5 categorias
exigidas pelo enunciado. Para cada consulta gera no `consultas/resultados_sparql.md`:
(1) descrição em português, (2) bloco de código SPARQL, (3) resultado em tabela.

Observação importante de modelagem: o `taim.owl` é salvo com os fatos AFIRMADOS
(sem materializar inferências do reasoner). Por isso as consultas "por classe" que
dependem da hierarquia usam o *property path* `rdf:type/rdfs:subClassOf*` para
percorrer as subclasses (ex.: toda `Capivara` também é `Animal`).

Execução (após as Fases 1–3):
    .\\venv\\Scripts\\python.exe scripts\\04_consultas_sparql.py
"""
