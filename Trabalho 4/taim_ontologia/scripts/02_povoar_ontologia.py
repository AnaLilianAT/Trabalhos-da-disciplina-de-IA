...
Carrega a TBox já construída (`taim.owl`) e gera >= 100 indivíduos coerentes:
espécies, habitats, rodovia/trechos, condições ambientais, fatores de risco e
~50 eventos de atropelamento. Os dados são SINTÉTICOS porém PLAUSÍVEIS, ancorados
em fatos reais (a BR-471 corta a ESEC Taim; capivara é a espécie emblemática de
atropelamento; chuva/neblina reduzem a visibilidade).

Usa `random.seed(42)` para reprodutibilidade. Ao final roda o reasoner HermiT,
imprime a contagem de indivíduos e 3 exemplos concretos, e salva o `taim.owl`
com a ABox incluída.

Execução (após rodar a Fase 1):
    .\\venv\\Scripts\\python.exe scripts\\02_povoar_ontologia.py
"""
...
# Carrega via fileobj (evita problemas de URI file:// no Windows com espaços).
...
with onto:
    ...
    # =======================================================================
    # 1) ESPÉCIES / ANIMAIS (~15)
    # Cada animal: nomeComum, nomeCientifico, pesoMedioKg, habita.
    # =======================================================================
    ...
    # =======================================================================
    # 3) RODOVIA + TRECHOS (1 + 12)
    # BR-471 corta a ESEC Taim; faixa aprox. km 498–542.
    # =======================================================================
    br471 = onto.Rodovia("BR_471")

    # Fatores de risco precisam existir antes de ligar aos trechos -> criados
    # já a seguir; mas para clareza, criamos os fatores aqui.
    # ---- FATORES DE RISCO (~8) -------------------------------------------
    ...
    for km in kms:
        ...
        t.pertenceARodovia = br471          # funcional -> valor escalar
    ...
    # Garante que ALGUNS trechos sejam TrechoCritico (definição por equivalência:
    # proximoA some Banhado AND temFatorRisco some FatorProximidadeAgua).
    # O reasoner deverá classificá-los automaticamente como TrechoCritico.
    ...
    # =======================================================================
    # 4) CONDIÇÕES AMBIENTAIS (~12)
    # =======================================================================
    # Clima (4) com descrição, temperatura e visibilidade.
    ...
    # =======================================================================
    # 6) EVENTOS DE ATROPELAMENTO (~50)
    # Cada um: dataHora (funcional), envolveAnimal (>=1), ocorreEmTrecho (==1),
    # ocorreSobClima, ocorreNoPeriodo, ocorreNaEstacao, numeroAnimaisEnvolvidos.
    # =======================================================================
    def estacao_de(mes):
    ...
    for i in range(1, N_EVENTOS + 1):
        ...
        # Data/hora plausível ao longo de 2024 (atropelamentos concentram-se à
...
def main():
    ...
    print(f"Total de indivíduos criados: {total}  (mínimo 100)")
    assert total >= 100, "FALHA: menos de 100 indivíduos."
    ...
    # Salvamos só os fatos AFIRMADOS (sem inferências), para que o arquivo
    # abra "limpo" no Protégé e o grupo rode o reasoner lá por conta própria.
    ...
    # ---- 3 exemplos concretos (para colar no relatório) ------------------
    print("\n" + "-" * 60)
